...
/* Public opaque handle for internal fault structure. */
struct spires_fault_system {
	struct fault_system *impl; /* yes ahaha */
};
...
/* ---------------- Fault Simulation ---------------- */
// make sure reservoir is already created before calling.
spires_status spires_create_fault_system(const spires_reservoir *r, spires_fault_type fault_type, spires_fault_system **out_fs)
{
	if (!r || !out_fs)
		return SPIRES_ERR_INVALID_ARG;
	
	enum fault_type fault_type_internal = (enum fault_type)fault_type;
	struct fault_system *impl = create_fault_system(r->impl, fault_type_internal); // also initializes
	if (!impl)
		return SPIRES_ERR_INTERNAL;
	spires_fault_system *fs = malloc(sizeof(*fs));
	if (!fs) {
		free_fault_system(impl);
		return SPIRES_ERR_ALLOC;
	}
	fs->impl = impl;
	*out_fs = fs;
...
void spires_destroy_fault_system(spires_fault_system *fs)
{
	if (!fs)
    	return;
    if (fs->impl)
        free_fault_system(fs->impl);
    free(fs);
}

spires_status spires_generate_fault(spires_reservoir *r, const struct spires_fault_system *fs)
{
	switch(fs->impl->fault_type) {
		case NONE:
			break;
		case CLAMP_LOW:
			process_clamp_low(r->impl, fs->impl);
			break;
		default:
			fprintf(stderr, "Error generating fault: fault_type %d not recognized.\n", (int)fs->impl->fault_type);
			return SPIRES_ERR_INVALID_ARG;
	}
	return SPIRES_OK;
}
...
// TODO
spires_status spires_restore_fault(spires_reservoir *r, const spires_fault_system *fs)
{
	return SPIRES_OK;
}